...
- **快速分辨率切换：管理员直写路径（免 helper 转发）**：主程序以管理员身份运行时，EDID override 写入与显示器/适配器重启不再经计划任务 + 命名管道转发，本进程直接执行（`PrivilegedOps` 按进程提权状态自动选择直写/helper 转发，用户零操作）。预置前置守卫放行条件相应放宽为「自身是管理员 或 计划任务已注册」任一。**看门狗仍走 helper**——它是独立进程守护（主进程崩溃时自动还原 EDID 防黑屏），主进程无法自我守护，故直写路径下计划任务仍用于武装/解除看门狗。
- **免责声明文档 `docs/disclaimer.md`**：覆盖按「现状」提供、实测组合范围、安装包未签名（SmartScreen/SHA256 自验）、DRM 不可采、延迟适用场景、合规使用义务、局域网限制与第三方组件；官网页脚「文档」列新增「免责声明 / Disclaimer」链接（中英双语同步）。
...
- **预置误伤已在系统列表的自定义分辨率（2304×1440@165 报「预置验证失败」）**：「立即预置」此前对所有待预置模式一律强行 EDID 注入，不检查该模式是否已在系统模式列表。实机场景：用户已在 NVIDIA 控制面板创建 2304×1440@165（驱动已注册进系统列表），软件仍注入 EDID → 被驱动按 range limits 裁剪 → 误判「预置验证失败」。修复：`provision()` 预置前先按系统列表分流——已在列表的直接标 `Ready/System` 跳过注入（且不再要求提权/辅助组件），只对真正缺失的模式走 EDID 注入；纯系统列表场景免提权通过。
- **自定义高刷分辨率预置后被驱动裁剪出系统列表（2304×1440@165 等）**：两类修复。① `Auto`（native-blanking 继承）产物像素时钟超 DTD 编码上限 655.35MHz 时回退 CVT-RB2——此前超限 timing 只能写入 DisplayID Type VII，而 Windows 显示驱动栈普遍不把它枚举为系统模式；② 新增按显示器行频上限（EDID range limits）压缩垂直消隐——此前 RB2 的 460µs 最小 vblank 会把 2304×1440@165 行频推到 257kHz，超显示器 250kHz 上限而被驱动静默丢弃。现经 `generate_for_display` 统一走「继承→DTD 上限回退→行频压缩」三段，预置/校验共用。注：EDID 注入仅能加入「EDID 已声明分辨率的刷新率」，无法凭空新增分辨率（实机验证，与 CRU 一致）；全新分辨率需先经 NVIDIA 控制面板注册（M8 NVAPI CustomDisplay 路径预留，FFI 已按官方 nvapi.h 布局实现，本机 `TryCustomDisplay` 返回 `NVAPI_INVALID_DISPLAY_ID`，待 displayId 解析完善后启用）。
